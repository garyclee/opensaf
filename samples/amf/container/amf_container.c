...
int start_docker(void)
{
  static const char command[] = "systemctl start docker";

  // XXX real application would check error codes, etc.
  system(command);

  return 0;
}

int stop_docker(void)
{
  static const char command[] = "systemctl stop docker";
...
//static SaAmfHealthcheckKeyT my_healthcheck_key = {"Container1", 10};
...
static char contained_id[256] = { 0 };
...
static void contained_component_instantiate_callback(SaInvocationT invocation,
                                       const SaNameT *containedCompName)
{
	SaAisErrorT rc, response = SA_AIS_OK;
	static bool init = false;

	syslog(LOG_INFO, "=====Contained Instantiate Callback====>");
	syslog(LOG_INFO, "     comp:%s",  saAisNameBorrow(containedCompName));
	...
	if (!init) {
		...
		syslog(LOG_INFO, "contained started with id: %s", contained_id);
	}


        rc = saAmfResponse_4(my_amf_hdl, invocation, 0, response);
        if (rc != SA_AIS_OK) {
                syslog(LOG_ERR, "saAmfResponse_4 FAILED - %u", rc);
                exit(1);
        }
...
static void contained_component_cleanup_callback(SaInvocationT invocation,
                                               const SaNameT *containedCompName)
{
	SaAisErrorT rc;
	syslog(LOG_INFO, "=====Contained Clean Up Callback====>");
	syslog(LOG_INFO, "     comp:%s",  saAisNameBorrow(containedCompName));

	char command[256] = { 0 };

	sprintf(command, "docker stop %s", contained_id);
...
static SaAisErrorT amf_initialize(SaSelectionObjectT *amf_sel_obj)
{
	...
	amf_callbacks.saAmfComponentTerminateCallback =
        ...
        amf_callbacks.saAmfContainedComponentInstantiateCallback =
            contained_component_instantiate_callback;
        amf_callbacks.saAmfContainedComponentCleanupCallback =
            contained_component_cleanup_callback;


	rc = saAmfInitialize_4(&my_amf_hdl, &amf_callbacks, &api_ver);
	if (rc != SA_AIS_OK) {
