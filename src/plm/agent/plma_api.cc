...
#define m_PLM_VER_IS_VALID(ver)               \
  ((ver->releaseCode == PLMA_RELEASE_CODE) && \
   ((ver->majorVersion <= PLMA_MAJOR_VERSION) && (ver->majorVersion > 0)))
 ...
 ***************************************************************************/
SaAisErrorT plm_add_entity_addr_to_list(
    PLMA_RDNS_TRK_MEM_LIST **listHead, SaPlmReadinessTrackedEntityT *entities) {
  PLMA_RDNS_TRK_MEM_LIST *current, *new_mem;
  current = *listHead;

  TRACE_ENTER();
  new_mem = (PLMA_RDNS_TRK_MEM_LIST *)malloc(sizeof(PLMA_RDNS_TRK_MEM_LIST));
  if (!new_mem) {
    LOG_CR(
        "PLMA : PLMA_RDNS_TRK_MEM_LIST memory alloc failed,"
        "error val:%s",
        strerror(errno));
    return SA_AIS_ERR_NO_MEMORY;
  }
  new_mem->entities = entities;
  new_mem->next = NULL;
      ...
      if (group_info->trk_strt_stop) {
        if (msg->req_evt.agent_track.evt_type == PLMS_AGENT_TRACK_CBK_EVT) {
          if (client_info->track_callback.saPlmReadinessTrackCallback) {
            /** Invoke the callback */
            client_info->track_callback.saPlmReadinessTrackCallback(
                grp_hdl, msg->req_evt.agent_track.track_cbk.track_cookie,
                ...
                msg->req_evt.agent_track.track_cbk.change_step, SA_AIS_OK);
...
uint32_t plm_process_blocking_dispatch(PLMA_CLIENT_INFO *client_info) {
  ...
  /** Receive a message from the mailbox */
  while (NULL != (msg = (PLMS_EVT *)m_NCS_IPC_RECEIVE(&client_info->callbk_mbx,
                                                      NULL))) {
    /** Process the message here */
    if ((msg->req_res == PLMS_REQ) &&
        (PLMS_AGENT_TRACK_EVT_T == msg->req_evt.req_type)) {
 ...
 ***************************************************************************/
SaAisErrorT saPlmInitialize(SaPlmHandleT *plmHandle,
...

{
  ...
  if (!client_info) {
    LOG_CR(
        "PLMA : PLMA_CLIENT_INFO memory alloc failed,"
        "error val:%s",
        strerror(errno));
  ...
  proc_rc =
      plm_mds_msg_sync_send(plma_cb->mds_hdl, NCSMDS_SVC_ID_PLMA,
  ...
  if (!plm_init_resp) {
    ...
    *plmHandle = plm_init_resp->res_evt.hdl;
    ...
    proc_rc =
        ncs_patricia_tree_add(&plma_cb->client_info, &client_info->pat_node);
 ...
 * @return    : Refer to SAI-AIS specification for various return values.
 ***************************************************************************/
SaAisErrorT saPlmSelectionObjectGet(SaPlmHandleT plmHandle,
...
void add_entity_grp_to_list(PLMA_ENTITY_GROUP_INFO_LIST **listHead,
  ...
  PLMA_ENTITY_GROUP_INFO_LIST *current, *new_mem;
  current = *listHead;

  TRACE_ENTER();
  new_mem = (PLMA_ENTITY_GROUP_INFO_LIST *)malloc(
      sizeof(PLMA_ENTITY_GROUP_INFO_LIST));
  if (!new_mem) {
    LOG_CR(
        "PLMA: PLMA_ENTITY_GROUP_INFO_LIST memory alloc failed,"
        "error val:%s",
        strerror(errno));
    return;
  }
  new_mem->entity_group_info = entityNew;
  new_mem->next = NULL;
...
SaAisErrorT saPlmEntityGroupCreate(SaPlmHandleT plmHandle,
  ...
  if (!group_info) {
    LOG_CR(
        "PLMA: PLMA_ENTITY_GROUP_INFO memory alloc failed,"
        "error val:%s",
        strerror(errno));
    rc = SA_AIS_ERR_NO_MEMORY;
    goto end;
  ...
  plm_in_evt.req_evt.agent_grp_op.grp_evt_type = PLMS_AGENT_GRP_CREATE_EVT;
...
SaAisErrorT saPlmEntityGroupDelete(SaPlmEntityGroupHandleT entityGroupHandle) {
  ...
  ncs_patricia_tree_del(&plma_cb->entity_group_info, &group_info->pat_node);
...
void add_entities_to_track_list(PLMA_RDNS_TRK_MEM_LIST **entity_list,
  ...
  PLMA_RDNS_TRK_MEM_LIST *new_mem, *temp;

  TRACE_ENTER();
  temp = *entity_list;
  new_mem = (PLMA_RDNS_TRK_MEM_LIST *)malloc(sizeof(PLMA_RDNS_TRK_MEM_LIST));
  if (!new_mem) {
    LOG_CR("PLMA : PLMA_RDNS_TRK_MEM_LIST memory alloc failed, error val:%s",
           strerror(errno));
    return;
  }
  new_mem->entities = entity;
  new_mem->next = NULL;
  if (temp == NULL) {
    *entity_list = new_mem;
  } else {
 ...
 *which will be passed as a parameter to all invocations of
...
SaAisErrorT saPlmReadinessTrack(
    SaPlmEntityGroupHandleT entityGroupHandle, SaUint8T trackFlags,
    SaUint64T trackCookie, SaPlmReadinessTrackedEntitiesT *trackedEntities) {
  PLMA_CB *plma_cb = plma_ctrlblk;
  PLMS_EVT plm_in_evt;
  PLMS_EVT *plm_out_res = NULL;
  SaAisErrorT rc = SA_AIS_OK;
  ...
  if (m_PLM_IS_SA_TRACK_CURRENT_SET(trackFlags)) {
    if (trackedEntities) {
      if ((trackedEntities->entities != NULL) &&
        ...
        LOG_ER(
            "PLMA: INVALID INPUT PARAM. entities pointer != NULL and "
            "numberOfEntities = 0 ");
        rc = SA_AIS_ERR_INVALID_PARAM;
        goto end;
      }
  ...
  if (m_PLM_IS_SA_TRACK_CURRENT_SET(trackFlags) && (trackedEntities)) {
    ...
    if (plm_out_res->res_evt.error != SA_AIS_OK) {
      if (SA_AIS_ERR_NO_SPACE == plm_out_res->res_evt.error)
        trackedEntities->numberOfEntities =
            plm_out_res->res_evt.entities->numberOfEntities;
  ...
  if (trackedEntities) {
    ...
    if ((!trackedEntities->entities) && (entities_num)) {
      trackedEntities->entities = static_cast<SaPlmReadinessTrackedEntityT *>(
          malloc(entities_num * sizeof(SaPlmReadinessTrackedEntityT)));
      add_entities_to_track_list(&group_info->rdns_trk_mem_list,
                                 trackedEntities->entities);
    }

    if (plm_out_res->res_evt.entities) {
      memcpy(trackedEntities->entities, plm_out_res->res_evt.entities->entities,
 ...
 *PLM Service in the SaPlmReadinessTrackCallbackT callback.
...
SaAisErrorT saPlmReadinessTrackResponse(SaPlmEntityGroupHandleT entityGrpHdl,
  ...
  if (plm_out_res) plms_free_evt(plm_out_res);
 ...
 *to be freed by using saPlmReadinessNotificationFree() function.
 ...
 *the PLM Service library in the saPlmReadinessTrack() function and is to be
 *deallocated.
 * @return   : Refer to SAI-AIS specification for various return values.
 ***************************************************************************/
SaAisErrorT saPlmReadinessNotificationFree(
  ...
  proc_rc = validate_entities_ptr(&group_info->rdns_trk_mem_list, entities);
 ...
 *of health of an entity has changed.
 * @param[in]    : plmHandle - The handle which was obtained by a previous
