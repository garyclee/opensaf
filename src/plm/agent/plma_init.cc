...
uint32_t plma_client_tree_init() {
  ...
  if (ncs_patricia_tree_init(&cb->client_info, &param) != NCSCC_RC_SUCCESS) {
...
uint32_t plma_client_tree_destroy() {
  ...
  while ((client_node = (PLMA_CLIENT_INFO *)ncs_patricia_tree_getnext(
    ...
    if (client_node != NULL) {
      ...
      if (ncs_patricia_tree_del(&cb->client_info, &client_node->pat_node) !=
          NCSCC_RC_SUCCESS) {
...
uint32_t plma_group_tree_destroy() {
  ...
  TRACE_ENTER();
  /* scan the entire handle db & delete each record */
  while ((grp_info_node = (PLMA_ENTITY_GROUP_INFO *)ncs_patricia_tree_getnext(
    ...
    if (grp_info_node != NULL) {
      ...
      if (ncs_patricia_tree_del(&cb->entity_group_info,
                                &grp_info_node->pat_node) != NCSCC_RC_SUCCESS) {
