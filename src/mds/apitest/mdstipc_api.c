...
bool tet_receiver(char *expected_buff, uint32_t buff_len, int msg_count)
{
	...
	while(counter < msg_count) {
		...
		if (ret > 0) {
			...
			if (msg != NULL) {
				memcpy(&received_buff[offset],msg->recvd_data, msg->recvd_len);
	...
	if (offset == buff_len) {
		if (memcmp(received_buff, expected_buff, buff_len) != 0) {
			...
			FILE *fp = fopen("/var/log/opensaf/mdstest_received_buff","wb");
...
__attribute__((constructor)) static void mdsTipcAPI_constructor(void)
{
  ...
  test_case_add(25, tet_create_named_VDEST_9, "Not able to Create a Named VDEST in MxN model with NULL name");
	...
	test_case_add(
		...
		"Receiver wait for drop normal message retransmit and receive all messages in order");
	test_case_add(
		27, tet_overload_tp_2,
		"Receiver wait for drop fragment message retransmit and receive all messages in order");
}
